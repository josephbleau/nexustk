...
module.exports = function (client, data) {
	...
	payload = payload.concat([0x00, 0x00]); /* x */
	payload = payload.concat([0x00, 0x00]); /* y */

	payload.push(data.width);
	payload.push(data.height);

	payload.push(0x00);

	payload.push(data.width);

	payload = payload.concat([0x00, 0x00, 0x00, 0x00]); /* checksum */
